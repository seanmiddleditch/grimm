...
namespace up {
    ...
    void CameraSystem::update(float frameTime) {
        ...
        space().entities().select<Transform, FlyCamera>([frameTime](EntityId, Transform& transform, FlyCamera& flyCam) {
            // apply movement to transform
            glm::vec3 const move = transform.right() * flyCam.relativeMovement.x +
                    transform.up() * flyCam.relativeMovement.y +
                transform.forward() * flyCam.relativeMovement.z;
