...
namespace up {
    ...
    public:
        ...
        virtual void invoke(CommandManager& commands, Command const* data) = 0;
    };
    ...
    public:
        ...
        virtual void invoke(CommandT const& command) { }
        virtual void invoke(CommandManager&, CommandT const& command) { invoke(command); }
    ...
    private:
        ...
        void invoke(CommandManager& commands, Command const* data) final {
            invoke(commands, *static_cast<CommandT const*>(data));
        }
    };
    ...
    public:
        ...
        auto condition(CommandId command, Command const* data) -> CommandResponse;
        auto status(CommandId command, Command const* data) -> CommandStatus;
        void invoke(CommandManager& commands, CommandId command, Command const* data);
    ...
    public:
        ...
        [[nodiscard]] bool refresh(uint64& lastVersion) noexcept;
        ...
        bool invoke(CommandId id, Command const* commad = nullptr);
        template <CommandType CommandT, typename... Args>
        bool invoke(Args&&... args) {
            CommandT const command(std::forward<Args>(args)...);
    ...
    void CommandScope::addHandler(Args&&... args) {
        uint32 const index = static_cast<uint32>(_handlers.size());
    ...
    void CommandManager::addCommand(Args&&... args) {
        ...
        uint32 const index = static_cast<uint32>(_commands.size());
